...
cat peppol.tsv | awk -F'\t' 'BEGIN { n = 0; }
{
  ...
  if (NR>1 && Kind!="" && "ABIE"!=Kind) {
    ...
    PropertyTerm = $11;
    DatatypeQualifier = $12;
    RepresentationTerm = $13;
    AssociatedObjectClassTermQualifier = $14;
    AssociatedObjectClass = $15;
    if (ObjectClassTermQualifier!="") {
      DEN = ObjectClassTermQualifier "_ ";
    } else {
      DEN = ""
    }
    DEN = DEN ObjectClassTerm ". "
    if ("ABIE"==Kind) {
      DEN = DEN "Details"
    } else {
      if (PropertyTermQualifier!="") {
        DEN = DEN PropertyTermQualifier "_ "
      }
      DEN = DEN PropertyTerm ". "
      if ("RFBIE"==Kind || "ASBIE"==Kind) {
        if (AssociatedObjectClassTermQualifier!="") {
          DEN = DEN AssociatedObjectClassTermQualifier "_ "
        }
        DEN = DEN AssociatedObjectClass
      } else {
        if (DatatypeQualifier!="") {
          DEN = DEN DatatypeQualifier "_ "
        }
        DEN = DEN RepresentationTerm
      }
    }
    ...
    if (PropertyTermQualifier!="") {
      print "$.data[" n "].PropertyTermQualifier",PropertyTermQualifier;
    }
    if (PropertyTerm!="") {
      print "$.data[" n "].PropertyTerm",PropertyTerm;
    }
    if (DatatypeQualifier!="") {
      print "$.data[" n "].DatatypeQualifier",DatatypeQualifier;
    }
    if (RepresentationTerm!="") {
      print "$.data[" n "].RepresentationTerm",RepresentationTerm;
    }
    if (AssociatedObjectClassTermQualifier!="") {
      print "$.data[" n "].AssociatedObjectClassTermQualifier",AssociatedObjectClassTermQualifier;
    }
    if (AssociatedObjectClass!="") {
      print "$.data[" n "].AssociatedObjectClass",AssociatedObjectClass;
    }
    datatype = $7;
    if (datatype!="") {
...
cat $Tmp-data | makrj.sh > list-peppol-entity.json #| sed "s/^\(\"[^\":,]*\":\),/\1null,/" > $Tmp-json
# --------------------------------------------------------------------
rm $Tmp-*
rm log/${0##*/}.$$.*
exit 0
# list-peppol-entity.sh